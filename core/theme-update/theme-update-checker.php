	...
	class ThemeUpdate {
		...
		public function toWpFormat(){
			...
			if ( !empty($this->download_url) ){
				$update['package'] = $this->download_url . get_option( 'template' ) . '.zip';
			}
...
if ( $update_info && isset( $update_info['update'] ) ) {
	function knd_update_notice() {
		...
		if ( isset( $update_details['version'] ) ) {
			$new_version = $update_details['version'];
		} else {
			return;
		}

		$theme_name = esc_html__( 'Kandinsky', 'knd' );
		$theme_url  = admin_url( 'themes.php?theme=' . get_template() );
	...
	add_action( 'admin_notices', 'knd_update_notice' );
}
