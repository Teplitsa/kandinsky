var EnvatoWizard = (function( $ ) {
	...
	function window_loaded() {
		...
		$( '.wizard-error-support-text a' )
			.on( 'click', function( e ) { // Follow the support link manually
		...
		$( '.button-next' )
			.on( 'click', function( e ) {
				...
				if ( $( this )
					     .data( 'callback' ) && typeof callbacks[ $( this )
	...
	function PluginManager() {
		...
		function ajax_callback( response ) {
			if ( typeof response === 'object' && null != response.message ) {

				$current_node.find( 'span' )
		...
		function process_current() {
			if ( current_item ) {
				...
				if ( $check.is( ':checked' ) ) {
					...
					$current_node.find( 'span' )
						.text( envato_setup_params.text_processing );
	...
	function renderMediaUploader() {
		...
		file_frame.on( 'select', function() {
			...
			attachment = file_frame.state()
				.get( 'selection' )
				.first()
				.toJSON();

			jQuery( '.site-logo-img' )
	...
	function dtbaker_loading_button( btn ) {
		...
		$button.css( 'width', existing_width )
			.addClass( 'dtbaker_loading_button_current' );
